...
type CustomerListItemDto = {
  id: number;
  customerCode: string;
  customerTypeID?: number; // bazı API'ler CustomerTypeId dönebilir
...
function fullName(p: CustomerListItemDto) {
...
function typeLabel(typeId?: number) {
  if (typeId === 1) return "Bireysel";
  if (typeId === 2) return "Kurumsal";
  if (typeId === 3) return "Şahıs Şirketi";
  return "-";
}
...
export default function CustomersClient() {
  ...
  const [items, setItems] = useState<CustomerListItemDto[]>([]);
  ...
  async function load(next?: { page?: number; pageSize?: number }) {
    ...
    try {
      ...
      const data = await apiGet<Paged<CustomerListItemDto>>(url);
  ...
  function openDeleteModal(p: CustomerListItemDto) {
    setErr(null);
    setDeleteTarget({ id: p.id, name: fullName(p) });
    setDeleteOpen(true);
  }
  ...
  async function confirmDelete() {
    ...
    try {
      await apiDelete(`/api/supervisor/customers/${id}`);
      ...
      <div className="overflow-hidden rounded-2xl border border-black/10 bg-white shadow-sm">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!loading &&
              items.map((p) => {
                const typeId = (p.customerTypeId ?? p.customerTypeID) as
                  | number
                  | undefined;
                return (
                  <tr
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-black/70">{p.email}</td>
                    <td className="px-4 py-3 text-black/70">
                      {p.phoneNumber ?? "-"}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="inline-flex items-center gap-2">
                        <Link
                          href={`/supervisor/customers/${p.id}`}
